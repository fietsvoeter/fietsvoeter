...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!post) return {}
  return {
    title: post.metaTitle, description: post.metaDescription, keywords: post.keywords,
...
export default function BlogPostPage({ params }: Props) {
  ...
  const breadcrumb = schemaBreadcrumb([{name:'Home',url:'/'},{name:cat?.label,url:`/categorie/${post.category}/`},{name:post.title,url:`/blog/${post.slug}/`}])
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-[1fr_300px] gap-10">
          <article>
            <nav className="text-xs text-gray-400 py-2 mb-5">
              <a href="/" className="text-blue-600 hover:underline">Home</a> <span className="mx-1">›</span>
              <a href={`/categorie/${post.category}/`} className="text-blue-600 hover:underline">{cat?.label}</a> <span className="mx-1">›</span>
            ...
            {post.featuredImage && <img src={post.featuredImage} alt={post.imageAlt||post.title} width={1200} height={630} className="w-full h-auto rounded-md mb-6" loading="eager"/>}
          ...
          <aside className="space-y-6">
            <div className="bg-gray-50 border border-gray-200 rounded p-5">
              ...
              <p className="text-xs text-gray-500 mb-3 leading-relaxed">Reviews en deals direct in je inbox.</p>
        ...
        {related.length > 0 && (
          <section className="mt-12 pt-8 border-t border-gray-200">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">{related.map(post=><BlogCard key={post.slug} post={post}/>)}</div>
          </section>
        )}
      </div>
    </>
