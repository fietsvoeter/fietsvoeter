...
export default async function BlogPostPage({ params }: Props) {
  ...
  try {
    ...
    content = <p>Artikel kan niet worden geladen.</p>
  }

  const cat     = CATEGORIES[post.category] || { label: post.category, color: '#E2001A' }
            ...
            {related.length > 0 && (
              <div className="border border-gray-200 rounded p-5">
                ...
                <ul className="space-y-2">
                  {related.map(r => (
                    <li key={r.slug}><a href={`/blog/${r.slug}/`} className="text-sm text-gray-600 no-underline hover:text-brand-red">→ {r.title}</a></li>
                  ))}
                </ul>
              </div>
            )}
            <div className="border border-gray-200 rounded p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(CATEGORIES).map(([slug, { label }]) => (
                  <a key={slug} href={`/categorie/${slug}/`} className="cat-pill text-xs">{label}</a>
                ))}
              </div>
            </div>
          </aside>
